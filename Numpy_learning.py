...
a = array([1,2,3]) # 一般都是用list来创建矩阵

## 定义数据类型
a = array([1,2,3], dtype= np.int)
print(a.dtype)
### 除了int
# NumPy中的基本数据类型
# 名称	描述
...
## 创建一个2行4列的随机矩阵，其中数值都是0~1的随机数
a = np.random.random((2,4))

## 创建一个从10到19的，2为步长有序的序列
a = np.arange(10,20,2)
a = np.arange(12) #从0到10的有序序列
a = np.arange(12).reshape((3,4)) #再把这个数列变成3行4列的矩阵
a = linspace(1,10,5) #生成线段，第一个数字是1，最后一个数字是10,一共5个数字
...
print(A[np.newaxis,:]) #在行上加一个维度；ps，无法对单行序列使用.T转置
...
print(np.split(A, 2, axis = 1)) #纵向切割，分成2块
print(np.split(A, 3, axis = 0)) #横向切割，分成3块
